		...
		<vx-card id="div-with-loading" class="vs-con-loading__container">
			<div class="flex flex-wrap mb-2">
				...
				<div class="w-full sm:w-1/3 px-1 flex justify-end" style="position: relative;">
					<vs-button @click="addRow()" class="mx-1 flex-shrink-0" color="dark" type="border">{{ $t('Add') }}</vs-button>
					<vs-button @click="saveDialog()" class="mx-1 flex-shrink-0" color="dark" type="border">{{ $t('Save') }}</vs-button>
					<vs-button @click="removeDialog()" class="mx-1 flex-shrink-0" color="dark" type="border">{{ $t('Delete') }}</vs-button>
					<vs-button @click="closeDialog()" class="mx-1 flex-shrink-0" color="dark" type="border">{{ $t('Close') }}</vs-button>
				</div>
			</div>
			...
			<div class="w-full flex flex-row mb-5">
				...
				<vs-input v-model="item2['comm_cd:comm2_nm']" style="width: 150px;" readonly />
			</div>
                    ...
                    <template slot-scope="{data}">
                        <vs-tr :data="tr" :key="index" v-for="(tr, index) in items1">
                            ...
                            <vs-td :data="data[index]['comm_cd:comm2_cd']">
								...
								<template slot="edit">
									<vs-input v-model="tr['comm_cd:comm2_cd']" class="inputx" placeholder="코드" />
								</template>
                            </vs-td>
                            ...
                            <vs-td :data="data[index]['comm_cd:comm2_nm']">
								...
								<template slot="edit">
									<vs-input v-model="tr['comm_cd:comm2_nm']" class="inputx" placeholder="코드명" />
								</template>
                            </vs-td>

							<vs-td>
								<vs-button @click="removeRow(index)" type="flat" color="danger" size="small" icon="clear" style="float: right"></vs-button>
							</vs-td>
			...
			<div class="flex flex-wrap justify-end mb-2">
                <vs-button @click="excel()" class="mx-1" color="dark" type="border" :disabled="items2.length <= 0">{{ $t('ToExcel') }}</vs-button>
            </div>

			<div class="overflow-y-auto" style="max-height: 300px;">
                <vs-table stripe :data="items2" v-model="item2" @selected="handleSelected">
...
export default {
	...
	methods: {
		...
		save () {
			...
			api.sync(this.item2['comm_cd:comm1_cd'], this.items1).then((res) => {
				this.spinner(false)
